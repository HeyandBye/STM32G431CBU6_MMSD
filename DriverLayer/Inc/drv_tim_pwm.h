 ...
 *  @note  调试时可关闭死区以排查波形问题, 实际驱动电机时必须使能 */
#define PWM_DEADTIME_ENABLE 0U

/** @brief 互补通道开关: 1=使能 CH1N/CH2N/CH3N（六路）, 0=仅主通道（三路）
 *  @note  互补通道 CHxN 是 CHx 的反相 PWM 信号（带死区插入）。
 *         当前使用 DRV8313 集成驱动芯片（内部已处理上下桥臂逻辑）, 设为 0。
 *         六路互补模式适合直接驱动 MOSFET 桥臂, 可降低谐波和转矩脉动。 */
#define PWM_ENABLE_N_CHANNELS 0U

/** @brief 自动重载值: f_PWM = 170MHz/(2×4250) ≈ 20.0kHz, 周期 50µs */
#define PWM_ARR            4249U

/** @brief 占空比最大值 = ARR（100% 上管占空比） */
#define PWM_DUTY_MAX       4249U

/** @brief 占空比最小值 = 0（0% 上管占空比, 下管 100%） */
#define PWM_DUTY_MIN       0U

/** @brief 死区 DTG 值: 34 × 5.882ns ≈ 200ns（T_dts = 1/170MHz ≈ 5.882ns） */
 ...
 * @note    必须在 MX_TIM1_Init 之后调用。仅写 BDTR 死区位,
 *          不动 CubeMX 已配好的 GPIO / 时基 / 通道配置。
 */
 ...
 * @note    MOE=0 → 所有 PWM 输出立即进空闲态（LOW）。
 *          定时器和通道保持配置, 故障解除后重新置位 MOE 即可恢复。
 ...
 * @param   b  B 相 CCR 值
 * @param   c  C 相 CCR 值
 * @note    自动钳位到安全范围, ISR 中可安全调用
 */
void drv_tim_pwm_set_duty(uint16_t a, uint16_t b, uint16_t c);

/**
 * @brief   设置三相占空比（浮点数）
 * @param   a  A 相占空比 [0.0, 1.0]（0.0=下管全通, 0.5=50%, 1.0=上管全通）
 * @param   b  B 相占空比
 * @param   c  C 相占空比
 */
