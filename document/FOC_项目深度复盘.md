...
sequenceDiagram
    ...
    Note over CPU: ⑦ Position_Init(Kp=0.10,Ki=0,Kd=0.03) → Position_Start
    ...
    loop 每 50µs (20kHz)
        ...
        ADC->>ADC: PI(Id→Vd): Kp=2.0,Ki=0.1,限幅±12V
        ADC->>ADC: PI(Iq→Vq): Kp=2.0,Ki=0.1,限幅±12V
    ...
    loop 主循环 (while(1))
        CPU->>CPU: LED心跳 1Hz
        Note over CPU: POS_AUTO_STEP=1: 每1s pos_cmd+=273(6°)
...
flowchart TB
    ...
    subgraph 控制层["🎛️ 三环串级控制"]
        PARK -->|"Id, Iq 反馈"| PI_ID["PID_Id<br/>Kp=2.0 Ki=0.1 Kd=0 Kr=1.0<br/>限幅±12V 积分限幅±12V"]
        PARK -->|"Iq 反馈"| PI_IQ["PID_Iq<br/>Kp=2.0 Ki=0.1 Kd=0 Kr=1.0<br/>限幅±12V 积分限幅±12V"]
        ...
        UNWRAP2 -->|"反馈"| PI_POS["PID_Pos<br/>Kp=0.10 Ki=0 Kd=0.03<br/>限幅±500RPM→speed_ref"]
    end
...
取 273 → 实际步进 = 273/16384×360 = 5.998° ≈ 6°

为什么是 273 而非 274?
  273/16384×360 = 5.998° → 每步误差 0.002°, 60步(一圈)累积误差 0.12°
  274/16384×360 = 6.021° → 每步误差 0.021°, 60步累积误差 1.26°
  ↓
  273 更接近真值，长时间运行累积误差更小
```

> **代码变更说明**：早期版本曾使用 2731 LSB/步（60°/步, 每500ms一步），
> 后调整为 273 LSB/步（6°/步, 每1s一步），更适合精细位置控制演示。
...
### 🔢 变量: 电流环 `Kp=4.0/2.0, Ki=0.3/0.1`

```mermaid
flowchart TB
    subgraph Kp推导["Kp=4.0/2.0 的来历"]
        A["电机绕组 R=5.6Ω L≈281µH"] --> B["电气时间常数<br/>τ=L/R=50µs"]
        B --> C["1A 误差需要多大电压修正?"]
        C --> D["稳态: V=I×R → 1A×5.6Ω=5.6V"]
        D --> E["Kp 至少 5.6 才能 1:1 抵消"]
        E --> F1["电流模式: Kp=4.0<br/>1A误差→4V修正<br/>+积分补偿剩余1.6V"]
        E --> F2["位置模式: Kp=2.0<br/>1A误差→2V修正<br/>+积分补偿剩余3.6V"]
        F2 --> G2["↓ 减小ADC噪声放大<br/>静止时Id/Iq抖动更小"]
    end

    subgraph Ki推导["Ki=0.3/0.1 的来历"]
        G["Ki 经验范围: Kp/10 ~ Kp/100"] --> H1["电流: 4.0/13.3=0.3"]
        H1 --> I1["Ti = Kp/Ki = 13.3 周期 = 667µs"]
        I1 --> J1["积分建立时间 ~667µs"]
        G --> H2["位置: 2.0/20=0.1"]
        H2 --> I2["Ti = Kp/Ki = 20 周期 = 1ms"]
        I2 --> J2["积分建立略慢<br/>但噪声抑制更好"]
    end

    subgraph 失败经历["调参血泪史"]
        L["Kp=4.0 Ki=4.0"] --> M["❌ 剧烈振荡<br/>积分太强,超调>50%"]
        N["Kp=4.0 Ki=2.0"] --> O["❌ 仍然振荡<br/>积分仍太强"]
        P["Kp=4.0 Ki=0.3"] --> Q["✅ 电流模式稳定<br/>Iq跟踪良好"]
        P --> R["位置模式→Kp=2.0 Ki=0.1<br/>✅ 静止抖动更小"]
    end
```

**为什么电流环积分限幅要覆盖为 ±12V**：绕组电感 281µH 在 12V 下的电流上升率 = 12V/281µH = 42.7 A/ms。50µs 内只能建立 2.1A。如果积分限幅只有 30%（±3.6V），积分项不足以补偿电感压降。

> **位置模式差异**：位置模式中电流环 PI 增益从 Kp=4.0/Ki=0.3 降低到 Kp=2.0/Ki=0.1，主要目的是减小 ADC 噪声放大——静止锁轴时 Id/Iq 抖动更小，电机更安静。

---

### 🔢 变量: 位置环 `Kp=0.10, Ki=0, Kd=0.03`
...
为什么 Kd=0.03?
  ① 轻微微分提前制动，减少位置过冲
  ② 100Hz 下微分噪声小，α=0.1 滤波充分抑制
  ③ 配合 P 使收敛更快，又不引入 I 的振荡风险
...
| 环路 | Kp | Ki | Kd | Kp/Ki 比值 | TI(等效) | 限幅 | Kr |
|------|-----|-----|-----|-----------|----------|------|-----|
| **电流 Id** | 2.0 V/A | 0.1 | 0 | 20.0 | 1ms | ±12V | 1.0 |
| **电流 Iq** | 2.0 V/A | 0.1 | 0 | 20.0 | 1ms | ±12V | 1.0 |
| **速度** | 0.002 A/RPM | 0.001 | 0 | 2.0 | 2ms | ±1A | 1.0 |
| **位置** | 0.10 RPM/LSB | **0** | **0.03** | ∞ | ∞ | ±500RPM | 1.0 |

> Kp/Ki 比值 = 等效积分时间常数（离散周期数）。电流环 ~20 周期快速消除静差，速度环 ~2 周期偏比例主导，位置环 P+D（Kd=0.03）加速收敛。
...
|------|------|-----------|
| ADC 转换 | ~6µs | 12% |
| 编码器 SPI 读取（2帧协议） | ~10µs | 20% |
| sin/cos 查表 ×2 | ~0.3µs | <1% |
| Clarke+Park+PI×2+InvPark | ~1.5µs | 3% |
| SVPWM | ~1.5µs | 3% |
| 角度计算+故障检测+其他 | ~1.3µs | 3% |
| PWM CCR 写入 | ~0.2µs | <1% |
| **总控制算法 (FOC_Current_Step)** | **~4.8µs** | **~10%** |
| ADC+SPI 等待 & 中断延迟 & 其他开销 | ~12µs | 24% |
| **全流程(触发ADC→输出PWM)** | **~33µs** | **~66%** |

> CPU 余量 ~34%，仍足够处理 CAN 通信、位置规划、在线参数整定等。
